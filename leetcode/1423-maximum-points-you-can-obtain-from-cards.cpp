...
Your score is the sum of the points of the cards you have taken. Given the 
integer array cardPoints and the integer k, return the maximum score you can 
obtain.
...
public:
    int maxScore(vector<int>& cardPoints, int k) {
      ...
      for (int j=0; j<k; j++){
        sum += cardPoints[front+j];
      }
      max = sum;
      
      for (int i=0; i<k; i++){
        sum += -cardPoints[front%size] + cardPoints[(front + k)%size];
        if (sum > max){
          max = sum;
        }
        front++;
