...
course ai. Given the total number of courses numCourses and a list of the 
...
class Solution {
    vector<int> vis; //0:not visited, 1: visiting, 2: visited
    vector<int> ord;
    bool isCycle;
    int pSize;
public:
    ...
    void dfs(int c, vector<vector<int>>& prerequisites){
      if (vis[c] == 2 || isCycle){ // already visited or cycle flag is on
        return;
      }
      else if (vis[c] == 1){ // visiting, means a cycle exists
        isCycle = true;
        return;
      }
      else {
        vis[c] = 1;
        for(int i=0; i<pSize; i++){ //visit child first
          if(prerequisites[i][0] == c){
            dfs(prerequisites[i][1], prerequisites);
          }
        }
        vis[c] = 2;
        ord.push_back(c); //push into order
      }
    }
};