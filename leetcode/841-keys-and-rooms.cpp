...
and forth between rooms freely. Return true if and only if you can enter 
every room.
...
public:
    bool canVisitAllRooms(vector<vector<int>>& rooms) {
      ...
      dfs(0, s, rooms, visited);
      return s.size() == rooms.size();
    }
  
    void dfs(int idx, vector<int>& s, vector<vector<int>>& rooms, 
                    vector<bool>& visited){
      //if visited already or stack is full, exit
      if(visited[idx] || s.size() == rooms.size()){
        return;
      }
      s.push_back(idx);
      visited[idx] = true;
